...
namespace OrderManage
{
    ...
    {
        public List<Order> orderList { get; private set; }      //订单链表
        ...
        {
            var orderSearched = from o in orderList
                                where o.customer.id == customer.id
                                orderby o.totalPrice
                                select o;
            return orderSearched.ToList();
        }
        ...
        public bool AddOrder(Order orderToAdd)
        {
            ...
            if (orderToAdd.id <= 0 || isRepeat)   //订单号无效或订单已在订单链表中
            {
                return false;
            }
            else
        ...
        //将所有订单序列化为XML文件,filePath是XML文件的路径
